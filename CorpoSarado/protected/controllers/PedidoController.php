...
class PedidoController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Pedido']))
		{
			...
			$item->cod_produto = Produto::model()->findByPk($key);
			$item->valorunitario=$_POST['Pedido']['valorunitario'];
			if($model->save())
				$this->redirect(array('view','id'=>$model->cod_pedido));
		}
	...
	public function actionUpdatebasket() {
        ...
        foreach ($_POST["quantidade"] as $key => $value) {
            $model = Item::model()->findByPk($key);
            $model->quantidade = $value;
            $model->save(false);
        }
        $this->redirect(Yii::app()->createUrl('atualizar'));
    }
